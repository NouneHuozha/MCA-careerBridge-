...
export default async function HomePage() {
  ...
  return <div className="overflow-hidden">
    <section className="relative border-b border-forest-200/70 bg-[#f1f7f2]">
      ...
      <div className="cb-container relative grid min-h-[610px] items-center gap-10 py-12 lg:grid-cols-[minmax(0,.82fr)_minmax(0,1.18fr)] lg:gap-3 lg:py-16">
        <div className="relative z-10 max-w-2xl">
          ...
          <h1 className="animate-rise delay-1 mt-6 max-w-[10ch] text-[clamp(3rem,5.5vw,5.7rem)] font-semibold leading-[.96] tracking-[-.07em] text-forest-900">Your next step starts with you.</h1>
          <p className="animate-rise delay-2 mt-6 max-w-[46ch] text-lg leading-relaxed text-ink-600 sm:text-xl">CareerBridge helps you understand yourself, explore real possibilities and find a route that feels right — without telling you what to choose.</p>
          <p className="animate-rise delay-2 mt-4 max-w-[48ch] text-sm leading-relaxed text-forest-700"><span className="font-semibold">Made for students in Nagaland.</span> Find local routes, institutions, scholarships and support without having to know the right answer first.</p>
        ...
        <div className="relative mx-auto w-full max-w-[820px] lg:-mr-16 lg:scale-[1.04]">
      ...
      <div className="cb-container relative pb-8"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl border border-forest-200/70 bg-white/60 px-4 py-3 text-sm text-ink-600"><span className="font-semibold text-forest-800">Explore with context</span><br />Not just a list of job titles.</div><div className="rounded-2xl border border-forest-200/70 bg-white/60 px-4 py-3 text-sm text-ink-600"><span className="font-semibold text-forest-800">Compare your options</span><br />See different ways forward.</div><div className="rounded-2xl border border-forest-200/70 bg-white/60 px-4 py-3 text-sm text-ink-600"><span className="font-semibold text-forest-800">Choose your next move</span><br />Small steps count too.</div></div></div>
    </section>

    <section className="cb-container py-16 sm:py-20" aria-labelledby="starting-point-title">
    ...
    <section className="cb-container py-16 sm:py-20" aria-labelledby="explore-title"><div className="flex flex-wrap items-end justify-between gap-6"><SectionHeading eyebrow="A whole platform, in one place" title={<span id="explore-title">Explore what could come next.</span>} description="Start with a question, a place, a course or a career field. Follow the thread that feels useful." /><Link href="/explore" className="cb-button cb-button-secondary shrink-0">Browse everything<ArrowRight aria-hidden className="h-4 w-4" /></Link></div><div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{platformLinks.map((item) => <Link href={item.href} key={item.href} className="group flex items-center gap-4 rounded-2xl border border-ink-200 bg-white p-4 transition hover:-translate-y-0.5 hover:border-forest-300 hover:bg-forest-50"><span className={`grid h-12 w-12 shrink-0 place-items-center rounded-2xl ${accentSurface[item.accent]}`}><item.icon aria-hidden className="h-5 w-5" /></span><span className="min-w-0"><span className="block font-semibold text-ink-900">{item.label}</span><span className="mt-1 block text-sm text-ink-500">{item.detail}</span></span><ArrowRight aria-hidden className="ml-auto h-4 w-4 shrink-0 text-forest-600 transition-transform group-hover:translate-x-1" /></Link>)}</div></section>
