...
export function InstitutionJourneyShell({ children, current = 3 }: { children: ReactNode; current?: number }) {
  return <div className="flex min-w-0"><aside className="hidden lg:block lg:w-[245px] lg:shrink-0" aria-label="Your journey"><div className="sticky top-24 min-h-[calc(100vh-7rem)] overflow-hidden border-r border-[#dce9e5] bg-[#edf5ff] px-6 py-8"><p className="text-xl font-semibold tracking-[-.03em] text-forest-900">Your journey</p><p className="mt-2 text-sm text-ink-500">Step by step to your brighter tomorrow</p><ol className="mt-7">{institutionSteps.map((step, index) => { const done = index < current; const active = index === current; return <li key={step.label} className="relative flex min-h-[88px] gap-3">{index < institutionSteps.length - 1 && <span aria-hidden className={`absolute left-[15px] top-8 h-[70px] w-px ${index < current ? "bg-[#42a58c]" : "bg-[#c7d9ea]"}`} />}<span className={`relative z-10 mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full border-2 text-sm ${active ? "border-[#2877df] bg-[#2877df] text-white" : done ? "border-[#42a58c] bg-[#42a58c] text-white" : "border-[#cad9e7] bg-white text-ink-500"}`}>{active ? index + 1 : done ? "✓" : index + 1}</span><Link href={step.href} className={`-mt-0.5 block rounded-lg px-2 py-1 ${active ? "bg-white/70 text-[#2877df]" : "text-ink-800 hover:bg-white/70"}`} aria-current={active ? "step" : undefined}><span className="block text-sm font-semibold">{step.label}</span><span className="mt-1 block max-w-[150px] text-xs leading-relaxed text-ink-500">{step.detail}</span></Link></li>; })}</ol><div className="relative mt-8 -mx-6 min-h-[240px] overflow-hidden bg-[#dcecf7] px-8 py-8"><div aria-hidden className="absolute -bottom-12 -left-12 h-40 w-80 rounded-[50%] bg-[#b3d0e8]" /><p className="relative max-w-[150px] text-lg italic leading-tight text-[#54738d]">Stronger learners<br />A brighter Nagaland</p></div></div></aside><div className="min-w-0 flex-1">{children}</div></div>;
}
