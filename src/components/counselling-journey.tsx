...
const answerStages = counsellingStages.filter((stage) => stage.key !== "reflection");
  ...
  return <ol className={mobile ? "grid gap-1 p-2" : "mt-8"}>
    {counsellingStages.map((stage, index) => {
      ...
      const content = <>
        ...
        {isUpcoming && <LockKeyhole aria-hidden className="mt-1 h-3.5 w-3.5 shrink-0 text-[#a6b6c0]" />}
      </>;
...
function ProgressSummary({ currentSection, completedSections }: { currentSection: CounsellingStageKey; completedSections: CounsellingStageKey[] }) {
  const completedCount = completedSections.filter((key) => answerStages.some((stage) => stage.key === key)).length;
  const currentIndex = answerStages.findIndex((stage) => stage.key === currentSection);
  const sectionLabel = currentSection === "reflection" ? "Reflection ready" : `Section ${Math.max(1, currentIndex + 1)} of ${answerStages.length}`;
  const percent = currentSection === "reflection" ? 100 : Math.round(((completedCount + (currentIndex >= 0 ? 0.35 : 0)) / answerStages.length) * 100);
  return <div className="mt-6 rounded-2xl border border-[#cfe2ef] bg-white/75 p-3.5"><div className="flex items-center justify-between gap-3"><p className="text-[11px] font-bold uppercase tracking-[.14em] text-[#38718e]">{sectionLabel}</p><span className="text-xs font-semibold text-ink-400">{percent}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#dfeaf1]"><div className="h-full rounded-full bg-[#36a989] transition-all" style={{ width: `${Math.max(8, percent)}%` }} /></div><p className="mt-2 text-xs leading-relaxed text-ink-500">{currentSection === "reflection" ? "Your answers are ready to review." : "One section at a time. You can edit completed sections later."}</p></div>;
}
...
export function CounsellingJourneyShell({ children, currentSection, completedSections }: { children: ReactNode; currentSection: CounsellingStageKey; completedSections: CounsellingStageKey[] }) {
  return <><header className="cb-counselling-header"><Logo /><span className="hidden text-right text-xs leading-relaxed text-ink-400 sm:block">Your space to think<br /><span className="text-ink-300">One step at a time</span></span></header><div className="flex min-w-0"><CounsellingJourneySidebar currentSection={currentSection} completedSections={completedSections} /><div className="min-w-0 flex-1"><div className="cb-container pt-4 lg:hidden"><CounsellingJourneyMobile currentSection={currentSection} completedSections={completedSections} /></div>{children}</div></div></>;
}
