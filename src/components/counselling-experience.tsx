...
import { ArrowRight, BarChart3, Check, CircleHelp, Leaf, Monitor, Palette, RotateCcw, UsersRound, Wrench } from "lucide-react";
...
export function CounsellingExperience({ initial, focusKey }: { initial: State; focusKey?: string }) {
  ...
  return <div className="mx-auto max-w-[1110px]">
    ...
    {question ? <form className="cb-counselling-form" onSubmit={(event) => { event.preventDefault(); void submit(); }}><fieldset disabled={pending}>
      ...
      <div className="cb-counselling-actions mt-8 flex flex-col gap-4 border-t border-ink-100 pt-4 sm:flex-row sm:items-center"><button type="button" disabled={pending} onClick={() => void submit("answer", true)} className="text-sm font-semibold text-[#6048b4] underline underline-offset-4">I&apos;m not sure yet</button><Link href={focusKey ? "/profile" : "/profile#my-answers"} className="text-sm font-semibold text-ink-500 underline underline-offset-4">Save and leave</Link><Button type="submit" disabled={pending} size="lg" className="min-w-36 sm:ml-auto">{pending ? "Saving…" : focusKey ? "Save changes" : "Continue"}<ArrowRight className="h-4 w-4" /></Button></div>
